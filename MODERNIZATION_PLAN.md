...
| 6 final                | `db66083`, `2424e6d` | express 5, commander 15, supertest 7, patch/minor sweep, tiny-emitter → EventTarget, Bun-native `scripts/clean.ts`, `vite.config.mts`, CI split into 4 ubuntu jobs (lint / typecheck-build / test / audit); standalone `--format=cjs` workaround removed (ink 7 needs ESM)                                                 |
| 7 Bun-native server    | _this commit_        | measure server rewritten on `Bun.serve` (HTTP + static + WebSocket upgrade) and native `WebSocket` on the client; express, cors, socket.io, socket.io-client, supertest and their `@types` dropped; typed JSON-frame protocol in `src/socket/` keeps the event maps, single-client eviction and pre-connect emit buffering |
...
- ~~Optional: measure server express → `Bun.serve`/Hono later; socket.io is the blocker to validate first — not part of this plan's scope.~~ Done in phase 7: `Bun.serve` + native WebSockets, no framework.
